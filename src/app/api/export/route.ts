...
export async function GET(req: NextRequest) {
  ...
  if (format === "pdf") {
    return exportPdf(timeline, name);
  }
...
async function exportPdf(timeline: YearBlock[], name: string): Promise<NextResponse> {
  ...
  const { default: ReactPDF, Document, Page, Text, View, StyleSheet } =
  ...
  const docElement = e(Document, {
    ...
    e(Page, { size: "A4", style: styles.page },
      e(View, { style: styles.cover },
        e(Text, { style: styles.coverName }, name),
    ...
    ...timeline.map((block) =>
      e(Page, { key: block.year, size: "A4", style: styles.page },
        ...
        e(Text, { style: styles.yearTagline }, block.tagline),
        ...block.achievements.map((a) =>
          e(View, { key: a.id, style: styles.eventCard },
            e(Text, { style: styles.eventTitle }, a.title),
            a.heading2
              ? e(Text, { style: styles.eventSubtitle }, a.heading2)
              : null,
            a.body ?? a.description
              ? e(Text, { style: styles.eventBody }, a.body ?? a.description)
              : null,
            a.categories?.length
              ? e(Text, { style: styles.eventCategories },
                  a.categories.map((c) => c.toUpperCase()).join("  ·  ")
                )
              : null,
          )
        ),
        e(Text, { style: styles.footer }, `${name} · HeroPortfolio.com`),
      )
    ),
  );

  const pdfBuffer = await ReactPDF.renderToBuffer(docElement);

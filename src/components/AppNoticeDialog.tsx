...
> = {
  success: {
    ...
    iconWrap: "app-notice-icon app-notice-icon-success border-green-500/25 bg-green-500/10 text-green-300",
    primaryBtn:
      "app-notice-btn app-notice-btn-success border-green-500/45 bg-green-500/20 text-green-50 hover:bg-green-500/30 focus-visible:outline-green-300/60",
  },
  error: {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" className="size-7" aria-hidden>
    ...
    iconWrap: "app-notice-icon app-notice-icon-error border-red-500/25 bg-red-500/10 text-red-300",
    primaryBtn:
      "app-notice-btn app-notice-btn-error border-red-500/40 bg-red-500/15 text-red-50 hover:bg-red-500/25 focus-visible:outline-red-300/60",
  },
  info: {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" className="size-7" aria-hidden>
    ...
    iconWrap: "app-notice-icon app-notice-icon-info border-sky-500/25 bg-sky-500/10 text-sky-300",
    primaryBtn:
      "app-notice-btn app-notice-btn-info border-sky-500/40 bg-sky-500/15 text-sky-50 hover:bg-sky-500/25 focus-visible:outline-sky-300/60",
  },
};
      ...
      {open && (
        ...
        >
          <button
            type="button"
            aria-label="Close"
            className="app-notice-backdrop absolute inset-0 bg-dusk-950/85 backdrop-blur-md"
            onClick={onClose}
          />
          <motion.div
          ...
          >
            ...
            <div className="relative p-7 sm:p-8">
              ...
              {stats && stats.length > 0 ? (
                <div className="mt-5 grid grid-cols-2 gap-2.5">
                  {stats.map((stat) => (
                    <div
                      key={stat.label}
                      className="app-notice-stat rounded-xl border border-dusk-600/70 bg-dusk-900/55 px-3.5 py-3 text-center"
                    >
              ...
              {warnings && warnings.length > 0 ? (
                <div className="app-notice-warnings mt-4 rounded-xl border border-amber-500/25 bg-amber-500/8 px-3.5 py-3">
                  <p className="app-notice-warnings-title text-[10px] font-semibold uppercase tracking-[0.12em] text-amber-200/90">
                    Notes
                  </p>
